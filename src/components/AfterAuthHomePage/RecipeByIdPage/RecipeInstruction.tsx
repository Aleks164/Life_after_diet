...
export const RecipeInstruction = ({ recipe }: RecipeType) => {
    ...
    const sorryText = <p>Sorry, but instruction for this recipe don't wrote yet</p>;
    ...
    return (
        <div className="instructions">
            ...
            {instructionType === "ingredients" ?
                <><h3>Ingredients</h3><hr />{recipe.extendedIngredients.length > 0 ? <div>{recipe.extendedIngredients.map((item) => <><li key={item.id} >{item.original}</li><hr /></>)}</div> : sorryText}</> : ""}
        </div>
    )
};